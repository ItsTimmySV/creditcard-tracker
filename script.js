document.addEventListener('DOMContentLoaded', () => {
  ...
  // Load / Save
  const saveData = () => localStorage.setItem('creditCardData', JSON.stringify(cards));
  const loadData = () => {
  ...
  function renderCardDetails() {
    ...
    detailsContentEl.innerHTML = `
      ...
      <div class="stats-grid">
        <div class="stat-card"><h4>Balance actual</h4><p>$${balance.toFixed(2)}</p></div>
        <div class="stat-card"><h4>Crédito disponible</h4><p>$${availableCredit.toFixed(2)}</p></div>
        <div class="stat-card"><h4>Próximo corte</h4><p>${nextCutoff.toLocaleDateString()}</p></div>
        <div class="stat-card"><h4>Próximo pago</h4><p>${nextPayment.toLocaleDateString()} <small>(${daysUntilPayment} días restantes)</small></p></div>
      </div>
      <div class="transactions-section">
        <div class="transactions-header"><h3>Movimientos</h3></div>
        <ul class="transaction-list">
          ${card.transactions.length === 0 ? '<li>No hay movimientos.</li>' : card.transactions.map(tx => `
            <li class="transaction-item ${tx.amount >= 0 ? 'expense' : 'payment'}">
              <div><span class="description">${tx.description}</span><span class="date">${new Date(tx.date).toLocaleDateString()}</span></div>
